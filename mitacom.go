...
func Rse(grp string, mita *tgbotapi.BotAPI, update tgbotapi.Update, updates tgbotapi.UpdatesChannel) {
	...
	r := (string(fContent))
	msgg := tgbotapi.NewMessage(update.Message.Chat.ID, "твоё расписание на сегодня:\n"+r)
	mita.Send(msgg)
...
func Any(mita *tgbotapi.BotAPI, update tgbotapi.Update, dev int) {
	if dev == 0 {
		...
		msgwr := tgbotapi.NewMessage(update.Message.Chat.ID, "https://github.com/bla1z3198/MitaBot")
		msgwr.ReplyToMessageID = update.Message.MessageID
		mita.Send(msgwr)
	}
